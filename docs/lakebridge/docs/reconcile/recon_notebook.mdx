...
```
## Configure Dashboards using Notebook

The recommended way is to use the CLI following the instructions [here](/installation.mdx#configure-reconcile).
However, if you want to configure the metadata tables and dashboards using a notebook, you can use the following notebook.

<a href={useBaseUrl("lakebridge_reconcile/dashboards.html")} target="_blank" rel="noopener noreferrer">
