...
async def test_sets_env_variables(
...
async def test_receives_client_info(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
...
async def test_receives_process_id(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
...
async def read_log(marker: str, test_resources: Path) -> str:
    # TODO: Fix this; logs should not be generated amongst the resources in our source tree.
    log_path = test_resources / "lsp_transpiler" / "test-lsp-server.log"
...
async def test_server_loads_document(
    ...
    sample_path = test_resources / "lsp_transpiler" / "source_stuff.sql"
    await lsp_engine.initialize(transpile_config)
    lsp_engine.open_document(sample_path, read_text(sample_path))
    log = await read_log("open-document-uri", test_resources)
    assert f"open-document-uri={sample_path.as_uri()}" in log


async def test_server_closes_document(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
    ...
    log = await read_log("close-document-uri", test_resources)
    assert f"close-document-uri={sample_path.as_uri()}" in log


async def test_server_transpiles_document(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
    ...
    expected_source = (test_resources / "lsp_transpiler" / "transpiled_stuff.sql").read_text(encoding="utf-8")
...
async def test_server_transpiles_from_memory(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
    ...
    await lsp_engine.shutdown()
    transpiled_path = test_resources / "lsp_transpiler" / "transpiled_stuff.sql"
    assert result.transpiled_code == transpiled_path.read_text(encoding="utf-8")


async def test_server_transpiles_relative_path(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
...
async def test_client_translates_diagnostics(
    resource: str,
    errors: Sequence[TranspileError],
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
    assert transpile_config.source_dialect is not None
    sample_path = test_resources / "lsp_transpiler" / resource
    await lsp_engine.initialize(transpile_config)
...
async def test_server_transpiles_workflow(
    lsp_engine: LSPEngine,
    transpile_config: TranspileConfig,
    test_resources: Path,
) -> None:
    with TemporaryDirectory() as output_folder:
        ...
        assert transpile_config.source_dialect is not None
        sample_path = test_resources / "lsp_transpiler" / "workflow.xml"
        await lsp_engine.initialize(transpile_config)
        result = await lsp_engine.transpile(
            transpile_config.source_dialect, "databricks", sample_path.read_text(encoding="utf-8"), sample_path
        )
